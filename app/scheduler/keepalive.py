...
class KeepAlive:
    ...
    def __init__(self) -> None:
        ...
        self._interval = settings.KEEPALIVE_INTERVAL_MINUTES * 60
    ...
    def start(self) -> None:
        ...
        logger.info("KeepAlive started -> %s every %ss", self._base_url, self._interval)
        ...
        try:
            while not self._stop.is_set():  # type: ignore[union-attr]
                ...
                try:
                    ...
                    logger.warning("KeepAlive ping failed，%s 秒后重试", self._interval)
        except asyncio.CancelledError:
            raise
        except Exception:
