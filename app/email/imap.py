    ...
    for host in _HOSTS:
        try:
            ...
            if key in _pool and _pool[key]:
                for idx, client in enumerate(_pool[key]):
                    try:
                        resp = await client.noop()
                        if resp.result == "OK":
                            return _pool[key].pop(idx)
                        await client.logout()
                    except Exception:  # noqa: BLE001
                        try:
                            await _pool[key].pop(idx).logout()
                        except Exception:  # noqa: BLE001
                            pass
            # 池满 → 排队等待（简化：直接新建）
...
async def _return_connection(email: str, client: aioimaplib.IMAP4_SSL) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        _pool_count[key] = max(0, _pool_count.get(key, 0) - 1)
...
async def list_emails(
    ...
    client = None
    try:
        client = await _get_connection(account.email, access_token)
        ...
        if client:
            await _return_connection(account.email, client)
...
async def search_emails(
    ...
    try:
        ...
        # 用 IMAP SEARCH 按 SUBJECT/FROM 匹配（OR 前置语法）
